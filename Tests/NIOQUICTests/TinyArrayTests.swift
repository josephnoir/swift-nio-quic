...
final class TinyArrayTests: XCTestCase {
    private func _assertEqual(
        ...
        _ mutate: (inout TinyArray<Int>) -> Void,
    ...
    func testExpressibleByArrayLiteral() {
        XCTAssertEqual(Array([] as TinyArray<Int>), [])
        XCTAssertEqual(Array([1] as TinyArray<Int>), [1])
        XCTAssertEqual(Array([1, 2] as TinyArray<Int>), [1, 2])
        XCTAssertEqual(Array([1, 2, 3] as TinyArray<Int>), [1, 2, 3])
        XCTAssertEqual(Array([1, 2, 3, 4] as TinyArray<Int>), [1, 2, 3, 4])
        XCTAssertEqual(Array([1, 2, 3, 4, 5] as TinyArray<Int>), [1, 2, 3, 4, 5])
    }
    ...
    func testThrowingInitFromResult() {
        XCTAssertEqual(Array(try TinyArray<Int>(CollectionOfOne(Result<_, any Error>.success(1)))), [1])
        XCTAssertEqual(Array(try TinyArray([Result<_, any Error>.success(1)])), [1])
        XCTAssertEqual(Array(try TinyArray([Result<_, any Error>.success(1), .success(2)])), [1, 2])
        XCTAssertEqual(Array(try TinyArray([Result<_, any Error>.success(1), .success(2), .success(3)])), [1, 2, 3])
        XCTAssertEqual(
            Array(try TinyArray([Result<_, any Error>.success(1), .success(2), .success(3), .success(4)])),
            [1, 2, 3, 4]
        )
        XCTAssertEqual(
            Array(
                try TinyArray([Result<_, any Error>.success(1), .success(2), .success(3), .success(4), .success(5)])
            ),
            [1, 2, 3, 4, 5]
        )
        ...
        XCTAssertThrowsError(
            Array(try TinyArray<Int>([.success(1), .success(2), Result.failure(MyError()), .success(4)]))
        )
        XCTAssertThrowsError(
            Array(try TinyArray<Int>([.success(1), .success(2), .success(3), Result.failure(MyError())]))
        )
        XCTAssertThrowsError(
            Array(
                try TinyArray<Int>([.success(1), .success(2), .success(3), .success(4), Result.failure(MyError())])
            )
        )
    }
}
