...
final class QUICDatagramHandler: ChannelDuplexHandler {
    ...
    /// Installs the SwiftNetwork-backed transport once the peer's `max_datagram_frame_size` is known.
    ///
    /// - Parameters:
...
extension QUICDatagramHandler: QUICDatagramReaderProtocol {
    ...
    func read(datagram: NIOCore.ByteBuffer) {
        // The SwiftNetwork backend already checks that incoming packets
        // do not surpass our advertised limit and will (per RFC) close
        // the connection with a PROTOCOL_ERROR in that case.
