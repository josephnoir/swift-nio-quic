...
extension TinyArray {
    @inlinable
    internal init(_ elements: some Sequence<Element>) {
        self.storage = .init(elements)
    }

    @inlinable
    internal init(_ elements: some Sequence<Result<Element, some Error>>) throws {
    ...
    @inlinable
    internal mutating func append(contentsOf newElements: some Sequence<Element>) {
    ...
    @inlinable
    internal mutating func sort(by areInIncreasingOrder: (Element, Element) throws -> Bool) rethrows {
